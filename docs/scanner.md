...
**실시간 수급 추정** (실전투자 장중 전용): DB에 당일 수급 데이터가 없는 종목에 대해 `fetch_investor_estimate()`로 KIS `investor_trend_estimate` API(TR_ID: HHPTJ04160200)를 호출하여 보강. 외국인 09:30, 기관 11:20부터 가집계 데이터 제공.
