...
점수는 `config.SIGNAL_SCORES_MOM` / `config.SIGNAL_SCORES_REV`에 모드별 분리 정의. `grid_search`가 자동 갱신하므로 문서에 고정 점수를 기재하지 않음. 중립 플래그(MPGC, BBM, SGC)는 양쪽 dict에 독립 점수를 가짐.

### 신호 모드 분류

- **MOM** (추세추종): 모멘텀 플래그 >= 2개 & 반전보다 많을 때. 점수 = 모멘텀(MOM dict) + 중립(MOM dict)
- **REV** (반전): 반전 플래그 >= 2개 & 모멘텀보다 많을 때. 점수 = 반전(REV dict) + 중립(REV dict)
- **MIX** (혼합): 양쪽 모두 있을 때. 점수 = max(모멘텀+중립MOM, 반전+중립REV) → **PO 대상에서 제외** (아래 참고)
- **WEAK**: 그 외. PO 대상에서 제외
