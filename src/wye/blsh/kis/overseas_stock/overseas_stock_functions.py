...
def daytime_order(
    ...
    res = ka._url_fetch(
        api_url=api_url, ptr_id=tr_id, tr_cont="", params=params, postFlag=True
    )
    ...
    if res.isOK():
        ...
        if tr_cont in ["M", "F"]:
            ...
            return foreign_margin(
                cano, acnt_prdt_cd, "N", dataframe, depth + 1, max_depth
            )
        else:
            logger.info("Data fetch complete.")
            return dataframe
            ...
            if output_data:
                ...
                if dataframe3 is not None:
                    dataframe3 = pd.concat(
                        [dataframe3, current_data3], ignore_index=True
                    )
                else:
                    dataframe3 = current_data3
            else:
...
def order_resv(
    ...
    sll_buy_dvsn_cd: Optional[
        str
    ] = "",  # 매도매수구분코드 (ex. 아시아인경우만 사용, 01:매도,02:매수)
    rvse_cncl_dvsn_cd: Optional[
        str
    ] = "",  # 정정취소구분코드 (ex. 아시아인경우만 사용, 00:매도/매수)
    ...
    algo_ord_tmd_dvsn_cd: Optional[
        str
    ] = "",  # 알고리즘주문시간구분코드 (ex. TWAP, VWAP 주문에서만 사용, 02로 고정)
) -> pd.DataFrame:
    """
...
def trade_pbmn(
    ...
    if nday == "":
        raise ValueError(
            "nday is required (e.g. '0:당일, 1:2일, 2:3일, 3:5일, 4:10일, 5:20일전, 6:30일, 7:60일, 8:120일, 9:1년')"
        )
    ...
    if res.isOK():
        ...
        if tr_cont == "M":
            ...
            return countries_holiday(
                trad_dt, NK, FK, "N", dataframe, depth + 1, max_depth
            )
        else:
            logger.info("Data fetch complete.")
            return dataframe
...
def industry_price(
    ...
    if excd == "":
        raise ValueError(
            "excd is required (e.g. 'NYS', 'NAS', 'AMS', 'HKS', 'SHS', 'SZS', 'HSX', 'HNX', 'TSE')"
        )
...
def quot_inquire_ccnl(
    ...
    params = {"EXCD": excd, "TDAY": tday, "SYMB": symb, "AUTH": auth, "KEYB": keyb}
...
def period_rights(
    ...
    if inqr_dvsn_cd == "":
        raise ValueError(
            "inqr_dvsn_cd is required (e.g. '02:현지기준일, 03:청약시작일, 04:청약종료일')"
        )
...
def price(
    ...
    if res.isOK():
        ...
        if tr_cont == "M":
            logger.info("Calling next page...")
            ka.smart_sleep()
            return price(auth, excd, symb, env_dv, "N", dataframe, depth + 1, max_depth)
        else:
            logger.info("Data fetch complete.")
            return dataframe
    else:
...
def price_detail(
    ...
    if res.isOK():
        ...
        if tr_cont == "M":
            logger.info("Calling next page...")
            ka.smart_sleep()
            return price_detail(auth, excd, symb, "N", dataframe, depth + 1, max_depth)
        else:
            logger.info("Data fetch complete.")
            return dataframe
    else:
...
def rights_by_ice(
    ...
    if ncod == "":
        raise ValueError(
            "ncod is required (e.g. 'CN:중국,HK:홍콩,US:미국,JP:일본,VN:베트남')"
        )
...
def search_info(
    ...
    if res.isOK():
        ...
        if tr_cont == "M":
            logger.info("Calling next page...")
            ka.smart_sleep()
            return search_info(prdt_type_cd, pdno, "N", dataframe, depth + 1, max_depth)
        else:
            logger.info("Data fetch complete.")
            return dataframe
    else:
