...
def capture_uplowprice(
    ...
    if fid_div_cls_code == "":
        raise ValueError(
            "fid_div_cls_code is required (e.g. '0', '6', '5', '1', '2', '3')"
        )
...
def chk_holiday(
    ...
    params = {"BASS_DT": bass_dt, "CTX_AREA_FK": FK100, "CTX_AREA_NK": NK100}
...
def comp_interest(
    ...
    if res.isOK():
        ...
        if hasattr(res.getBody(), "output1"):
            ...
            if output_data:
                current_data1 = pd.DataFrame(
                    output_data if isinstance(output_data, list) else [output_data]
                )
                dataframe1 = (
                    pd.concat([dataframe1, current_data1], ignore_index=True)
        ...
        if hasattr(res.getBody(), "output2"):
            ...
            if output_data:
                current_data2 = pd.DataFrame(
                    output_data if isinstance(output_data, list) else [output_data]
                )
                dataframe2 = (
                    pd.concat([dataframe2, current_data2], ignore_index=True)
...
def comp_program_trade_daily(
    ...
    if fid_cond_mrkt_div_code == "":
        raise ValueError(
            "fid_cond_mrkt_div_code is required (e.g. 'J:주식,NX:NXT,UN:통합')"
        )
...
def comp_program_trade_today(
    ...
    if fid_cond_mrkt_div_code == "":
        raise ValueError(
            "fid_cond_mrkt_div_code is required (e.g. 'J:KRX,NX:NXT,UN:통합')"
        )
            ...
            if output_data:
                ...
                if dataframe1 is not None:
                    dataframe1 = pd.concat(
                        [dataframe1, current_data1], ignore_index=True
                    )
                else:
            ...
            if output_data:
                ...
                if dataframe2 is not None:
                    dataframe2 = pd.concat(
                        [dataframe2, current_data2], ignore_index=True
                    )
                else:
...
def estimate_perform(
    ...
    if res.isOK():
        ...
        if hasattr(res.getBody(), "output3"):
            ...
            if output_data:
                current_data3 = pd.DataFrame(
                    output_data if isinstance(output_data, list) else [output_data]
                )
                dataframe3 = (
                    pd.concat([dataframe3, current_data3], ignore_index=True)
        ...
        if hasattr(res.getBody(), "output4"):
            ...
            if output_data:
                current_data4 = pd.DataFrame(
                    output_data if isinstance(output_data, list) else [output_data]
                )
                dataframe4 = (
                    pd.concat([dataframe4, current_data4], ignore_index=True)
...
def exp_closing_price(
    ...
    if fid_input_iscd == "":
        raise ValueError(
            "fid_input_iscd is required (e.g. '0000', '0001', '1001', '2001', '4001')"
        )

    if fid_rank_sort_cls_code == "":
        raise ValueError(
            "fid_rank_sort_cls_code is required (e.g. '0', '1', '2', '3', '4')"
        )
...
def foreign_institution_total(
    ...
    if fid_rank_sort_cls_code == "":
        raise ValueError(
            "fid_rank_sort_cls_code is required (e.g. '0:순매수상위,1:순매도상위')"
        )

    if fid_etc_cls_code == "":
        raise ValueError(
            "fid_etc_cls_code is required (e.g. '0:전체,1:외국인,2:기관계,3:기타')"
        )
...
def hts_top_view(
    ...
    if res.isOK():
        ...
        if tr_cont == "M":
            logger.info("Calling next page...")
            ka.smart_sleep()
            return hts_top_view("N", dataframe, depth + 1, max_depth)
        else:
            logger.info("Data fetch complete.")
            return dataframe
    else:
...
def inquire_balance(
    ...
    * 당일 전량매도한 잔고도 보유수량 0으로 보여질 수 있으나, 해당 보유수량 0인 잔고는 최종 D-2일 이후에는 잔고에서 사라집니다.
    ...
    if afhr_flpr_yn == "":
        raise ValueError(
            "afhr_flpr_yn is required (e.g. 'N:기본값, Y:시간외단일가, X:NXT')"
        )
    ...
    if prcs_dvsn == "":
        raise ValueError(
            "prcs_dvsn is required (e.g. '00: 전일매매포함, 01:전일매매미포함')"
        )
...
def inquire_balance_rlz_pl(
    ...
    if fund_sttl_icld_yn == "":
        raise ValueError(
            "fund_sttl_icld_yn is required (e.g. 'N:포함하지 않음, Y:포함')"
        )
...
def inquire_daily_ccld(
    ...
    * 다만, 3개월 이전 체결내역 조회(CTSC9115R) 의 경우,
    ...
    if sll_buy_dvsn_cd == "":
        raise ValueError(
            "sll_buy_dvsn_cd is required (e.g. '00 : 전체 / 01 : 매도 / 02 : 매수')"
        )
...
def inquire_daily_itemchartprice(
    ...
    if fid_input_iscd == "":
        raise ValueError(
            "fid_input_iscd is required (e.g. '종목코드 (ex 005930 삼성전자)')"
        )
    ...
    if fid_input_date_2 == "":
        raise ValueError(
            "fid_input_date_2 is required (e.g. '조회 종료일자 (최대 100개)')"
        )

    if fid_period_div_code == "":
        raise ValueError(
            "fid_period_div_code is required (e.g. 'D:일봉 W:주봉, M:월봉, Y:년봉')"
        )
...
def inquire_daily_price(
    ...
    if fid_period_div_code == "" or fid_period_div_code is None:
        raise ValueError(
            "fid_period_div_code is required (e.g. 'D:(일)최근 30거래일, W:(주)최근 30주, M:(월)최근 30개월')"
        )
...
def inquire_price(
    ...
    if fid_input_iscd == "" or fid_input_iscd is None:
        raise ValueError(
            "fid_input_iscd is required (e.g. '종목코드 (ex 005930 삼성전자), ETN은 종목코드 6자리 앞에 Q 입력 필수')"
        )
    ...
    if res.isOK():
        ...
        if tr_cont == "M":
            ...
            return inquire_psbl_sell(
                cano, acnt_prdt_cd, pdno, "N", dataframe, depth + 1, max_depth
            )
        else:
            logger.info("Data fetch complete.")
            return dataframe
...
def inquire_time_overtimeconclusion(
    ...
    if fid_input_iscd == "" or fid_input_iscd is None:
        raise ValueError(
            "fid_input_iscd is required (e.g. '123456(ETN의 경우 Q로 시작 Q500001)')"
        )
    ...
    if res.isOK():
        ...
        if tr_cont == "M":
            ...
            return ksdinfo_bonus_issue(
                cts, f_dt, t_dt, sht_cd, "N", dataframe, depth + 1, max_depth
            )
        else:
            logger.info("Data fetch complete.")
            return dataframe
    ...
    if res.isOK():
        ...
        if tr_cont == "M":
            ...
            return ksdinfo_forfeit(
                sht_cd, t_dt, f_dt, cts, "N", dataframe, depth + 1, max_depth
            )
        else:
            logger.info("Data fetch complete.")
            return dataframe
    ...
    if res.isOK():
        ...
        if tr_cont == "M":
            ...
            return ksdinfo_mand_deposit(
                t_dt, sht_cd, f_dt, cts, "N", dataframe, depth + 1, max_depth
            )
        else:
            logger.info("Data fetch complete.")
            return dataframe
    ...
    if res.isOK():
        ...
        if tr_cont == "M":
            ...
            return ksdinfo_paidin_capin(
                cts, gb1, f_dt, t_dt, sht_cd, "N", dataframe, depth + 1, max_depth
            )
        else:
            logger.info("Data fetch complete.")
            return dataframe
    ...
    if res.isOK():
        ...
        if tr_cont == "M":
            ...
            return ksdinfo_pub_offer(
                sht_cd, cts, f_dt, t_dt, "N", dataframe, depth + 1, max_depth
            )
        else:
            logger.info("Data fetch complete.")
            return dataframe
    ...
    if res.isOK():
        ...
        if tr_cont == "M":
            ...
            return ksdinfo_rev_split(
                sht_cd, cts, f_dt, t_dt, market_gb, "N", dataframe, depth + 1, max_depth
            )
        else:
            logger.info("Data fetch complete.")
            return dataframe
...
def market_cap(
    ...
    if res.isOK():
        # 응답 데이터 처리
        current_data = (
            pd.DataFrame(res.getBody().output)
            if hasattr(res.getBody(), "output")
            else pd.DataFrame()
        )
        ...
        # 기존 데이터프레임과 병합
        dataframe = (
            pd.concat([dataframe, current_data], ignore_index=True)
            if dataframe is not None
            else current_data
        )
...
def market_value(
    ...
    if fid_input_option_2 not in ["0", "1", "2", "3"]:
        raise ValueError("입력 옵션2 확인요망!!!")
    if fid_rank_sort_cls_code not in [
        "23",
        "24",
        "25",
        "26",
...
def mktfunds(fid_input_date_1: str = "") -> pd.DataFrame:
    """
    국내 증시자금 종합 API입니다.
    ...
    params = {"FID_INPUT_DATE_1": fid_input_date_1}
...
def order_credit(
    ...
    if loan_dt == "" or loan_dt is None:
        raise ValueError(
            "loan_dt is required (e.g. '[신용매수] 오늘날짜(yyyyMMdd), [신용매도] 매도할 종목의 대출일자(yyyyMMdd)')"
        )
...
def order_resv_rvsecncl(
    ...
    ord_dvsn_cd: Optional[
        str
    ] = "",  # 주문구분코드 (ex. 00:지정가, 01:시장가, 02:조건부지정가, 05:장전 시간외)
    ord_objt_cblc_dvsn_cd: Optional[
        str
    ] = "",  # 주문대상잔고구분코드 (ex. 10 : 현금, 12 : 주식담보대출, ... 28 : 자기대주상환)
    loan_dt: Optional[str] = "",  # 대출일자
...
def pension_inquire_daily_ccld(
    ...
    if ccld_nccs_dvsn == "":
        raise ValueError(
            "ccld_nccs_dvsn is required (e.g. '%%: 전체, 01: 체결, 02: 미체결')"
        )
...
def program_trade_by_stock(
    ...
    if fid_cond_mrkt_div_code == "":
        raise ValueError(
            "fid_cond_mrkt_div_code is required (ex. J:KRX,NX:NXT,UN:통합)"
        )
...
def psearch_result(
    ...
    if seq == "":
        raise ValueError(
            "seq is required (e.g. '종목조건검색 목록조회 API의 output인 'seq'을 이용')"
        )
...
def search_info(
    ...
    if res.isOK():
        ...
        if tr_cont == "M":
            logger.info("Calling next page...")
            ka.smart_sleep()
            return search_info(pdno, prdt_type_cd, "N", dataframe, depth + 1, max_depth)
        else:
            logger.info("Data fetch complete.")
            return dataframe
    else:
    ...
    if res.isOK():
        ...
        if tr_cont == "M":
            ...
            return search_stock_info(
                prdt_type_cd, pdno, "N", dataframe, depth + 1, max_depth
            )
        else:
            logger.info("Data fetch complete.")
            return dataframe
...
def top_interest_stock(
    ...
    if fid_input_iscd not in ["0000", "0001", "1001", "2001"]:
        logger.error(
            "fid_input_iscd is required. (e.g. '0000', '0001', '1001', '2001')"
        )
        raise ValueError(
            "fid_input_iscd is required. (e.g. '0000', '0001', '1001', '2001')"
        )
    if fid_div_cls_code not in ["0", "1", "2", "3", "4", "5", "6", "7"]:
        logger.error(
            "fid_div_cls_code is required. (e.g. '0', '1', '2', '3', '4', '5', '6', '7')"
        )
        raise ValueError(
            "fid_div_cls_code is required. (e.g. '0', '1', '2', '3', '4', '5', '6', '7')"
        )
...
def volume_power(
    ...
    if fid_input_iscd not in ["0000", "0001", "1001", "2001"]:
        ...
        raise ValueError(
            "fid_input_iscd must be one of ['0000', '0001', '1001', '2001']."
        )
    if fid_div_cls_code not in ["0", "1", "2"]:
