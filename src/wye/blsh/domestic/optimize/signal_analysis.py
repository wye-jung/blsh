...
실행:
    ...
    uv run python -m wye.blsh.domestic.optimize.signal_analysis --mode SWING
...
# 헬퍼
# ─────────────────────────────────────────
def _factor_to_params(f) -> Params:
    return Params(
        invest_min_score=f.invest_min_score,
        atr_sl_mult=f.atr_sl_mult,
        atr_tp_mult=f.atr_tp_mult,
        max_hold_days_rev=f.max_hold_days,
        max_hold_days_mix=f.max_hold_days_mix,
        max_hold_days_mom=f.max_hold_days_mom,
        tp1_mult=f.tp1_mult,
        tp1_ratio=f.tp1_ratio,
        gap_down_limit=f.gap_down_limit,
        sector_penalty_threshold=f.sector_penalty_threshold,
        sector_penalty_pts=f.sector_penalty_pts,
        sector_bonus_pts=f.sector_bonus_pts,
    )
...
# 메인
# ─────────────────────────────────────────
def run(mode: str = "", years: int = 2, rebuild: bool = False, top: int = 5):
    # mode 미지정 시 환경변수 사용
    trade_mode = mode or TRADE_FLAG
    log.info(f"분석 모드: {trade_mode}  기간: {years}년")
    ...
    # 현재 활성 factor → Params
    if trade_mode == "DAY":
        from wye.blsh.domestic.factor import _DAY as _MODE_DICT
    else:
        from wye.blsh.domestic.factor import _SWING as _MODE_DICT

    from wye.blsh.domestic import Factor
    factor = Factor(_MODE_DICT)
    params = _factor_to_params(factor)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", default="", choices=["DAY", "SWING", ""],
                        help="분석 모드 (기본: 환경변수 TRADE_FLAG)")
    ...
    run(mode=args.mode, years=args.years, rebuild=args.rebuild, top=args.top)
